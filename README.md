...
| POST | `/v1/trade-finance-facility-fulfillment-arrangement/initiate` | Initiate |
| GET | `/v1/trade-finance-facility-fulfillment-arrangement` | Retrieve (list) |
| GET | `/v1/trade-finance-facility-fulfillment-arrangement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/trade-finance-facility-fulfillment-arrangement/{crId}/update` | Update |
| PUT | `/v1/trade-finance-facility-fulfillment-arrangement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/trade-finance-facility-fulfillment-arrangement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-trade-finance:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-trade-finance ./helm -n bian-operations
```

Exposed through the platform Gateway at path prefix `/sd-trade-finance` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
